    ...
    public bool AddItem(Employee employee)
    {
        ...
        using (var connection = new SqlConnection(_connectionString))
        {
            ...
            using (var command = new SqlCommand(commandText, connection))
            {
                ...
                {
                    connection.Open();
                    var id = Convert.ToDecimal(command.ExecuteScalar());
                    employee.SetId(Decimal.ToInt32(id));
                    yesOrNo = true;
                    return yesOrNo;
                }
                catch (Exception)
    ...
    public void UpdateEmployee(Employee employee)
    {
        using (var connection = new SqlConnection(_connectionString))
        {
            var commandText = "UPDATE Employee SET firstName = @firstName, lastName = @lastName, address = @address, salary = @salary, email = @email, "
                            + "nickname = @nickname, password = @password, workingHours = @workingHours, phoneNumber = @phoneNumber, department = @department WHERE id = @id";
            using (var command = new SqlCommand(commandText, connection))
            {
